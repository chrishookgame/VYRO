-- ============================================================
-- VYRO HARDEN ADVANCE LIVE BATTLE SERIES RPC
-- Hardening backend: impedir finalización prematura
-- ============================================================
...
begin
    ...
    -- --------------------------------------------------------
    -- Defensa autoritativa de finalización
    -- --------------------------------------------------------

    if v_battle.status <> 'active' then
        raise exception
            'Solo una batalla activa puede ser finalizada.';
    end if;

    if v_battle.ends_at is null then
        raise exception
            'La batalla activa no tiene una hora de finalización válida.';
    end if;

    if now() < v_battle.ends_at then
        raise exception
            'La batalla todavía no ha finalizado.';
    end if;
